...
getSM = function(matrix){
  ...
  for (i in 1:length(matrix[[1]])) { #i=1
    SM[i] = median(matrix[i,2], matrix[i,3], matrix[i,4])
  }
...
getDeepEnsemble = function(resultsTrain, resultsTest){
  ...
  train_set = subset(matrix, split == TRUE); head(train_set)
  valid_set = subset(matrix, split == FALSE); head(test_set)
